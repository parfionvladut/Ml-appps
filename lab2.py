...
def test_picture_nn(picture: np.ndarray):
    ...
    # print(f'norm results: {norm_results}')
    index = np.argmin(norm_results)
    print(f'Index: {index}')
    resultPic = pics_matrix[:, index].reshape((112, 92))
    # resultPic = resultPic.reshape((92,112))
    return resultPic
...
if __name__ == '__main__':
    # ORL path
    path = 'D:\\Other\\PyCharm projects\\scientific_computing_algorithms\\ORL'
    ...
    test_pic_s = input('Insert person number (s folder number): ')
    test_pic_img = input('Insert image number (1 to 10): ')
    test_pic = cv2.imread(f'{path}\\s{test_pic_s}\\{test_pic_img}.pgm', 0)
    choice = input('Write nn or knn: ')
    if choice == 'nn':
        result = test_picture_nn(test_pic)
